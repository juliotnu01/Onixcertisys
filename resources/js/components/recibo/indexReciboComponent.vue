...
<v-app>
    <!--<v-row>
        <v-col cols="12" xs="!2" sm="12" md="12" lg="12">
            <v-container fluid>
                <v-data-iterator :items="recibos" :items-per-page.sync="itemsPerPage" :page="page" :search="search" hide-default-footer>
                    <template v-slot:header>
                        <v-toolbar dark color="blue darken-3" class="mb-1">
                            <v-text-field v-model="search" clearable text solo-inverted hide-details prepend-inner-icon="mdi-search" label="Buscar recibo"></v-text-field>
                            <v-btn class="mx-2" fab dark icon @click.prevent="services.reciboServices.getlistRecibos()">
                    ...
                    <template v-slot:default="props">
                        <v-row justify="space-around" align="center">
                            <v-col v-for="item in props.items" :key="item.id" cols="12" sm="6" md="5" lg="5">
                                <v-card>
                                    <v-card-title>
                                        <v-row>
                                            ...
                                            <v-col cols="12" xs="12" sm="12" md="12" lg="12">
                                                <v-row>
                                                    <v-col cols="12" xs="12" sm="12" md="6" lg="6">
                                                        <v-autocomplete v-model="item.userSelected" :items="empleados" outlined dense item-text="nombre_completo" return-object label="Seleccionar empleado"></v-autocomplete>
                                                    </v-col>
                                                    <v-col cols="12" xs="12" sm="12" md="2" lg="2">
                                                        <v-btn text depressed color="primary" @click="
                                        ...
                                        <template v-slot:default>
                                            ...
                                            <tbody>
                                                <tr v-for="item in item.has_partidas" :key="item.name">
                                                    ...
                                                    <td class="text-center">
                                                        <v-alert color="primary" dark dense small outlined class="mt-5">
                                                            {{ item.tipo }}
                                                        </v-alert>
                                                    </td>
                                                    <td class="text-center">
                                                        ...
                                                        <div v-else>
                                                            <v-alert color="error" dark dense small class="mt-5" style="font-size: 12px">
                                                                Sin Asignar
                                                            </v-alert>
                                                        </div>
                                                    </td>
    ...
    <v-card>
        ...
        <v-row class="pa-4" justify="space-between">
            <v-col cols="6" v-if="Object.entries(this.recibos).length > 0">
                <v-treeview :active.sync="active" :search="search" :items="items" :load-children="fetchUsers" :open.sync="open" activatable color="warning" open-on-click transition>
                </v-treeview>
            </v-col>
                    ...
                    <v-card v-else :key="selected.id" class="pt-6">
                        ...
                        <v-row>
                            <v-col cols="12" xs="12" sm="12" md="12" lg="12">
                                <v-row>
                                    <v-col cols="12" xs="12" sm="12" md="6" lg="6">
                                        <v-autocomplete v-model="userSelected" :items="empleados" outlined dense item-text="nombre_completo" return-object label="Seleccionar empleado"></v-autocomplete>
                                    </v-col>
                                    <v-col cols="12" xs="12" sm="12" md="2" lg="2">
                                        <v-btn text color="primary" @click="imprimirReciboUser(selected, userSelected)">
                                            <v-icon>mdi-printer</v-icon>
                                        </v-btn>
                                    </v-col>
                                    <v-col cols="12" xs="12" sm="12" md="4" lg="4">
                                        <v-btn block color="primary" @click="imprimirRecibo(selected)">Imprimir recibo</v-btn>
                                    </v-col>
                                </v-row>
                            </v-col>
                        </v-row>
