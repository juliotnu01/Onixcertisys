...
<v-app>
    <v-dialog v-model="openDialog" max-width="2556" min-width="2556" persistent>
        <v-toolbar dark color="primary">
            ...
            <v-spacer></v-spacer>
            <v-btn text dark tile @click="AddCotizacion">
                <v-icon>mdi-plus</v-icon>Crear Nueva Cotizacion
            </v-btn>
        </v-toolbar>
        <v-card>
            <v-card-text>
                <v-form ref="f_mag">
                    <v-row align="center" justify="space-around" v-if="Object.entries(this.cotizacion_para_duplicar).length > 0">
                        <v-col cols="12" xs="12" sm="12" md="3" lg="3">
                            <v-autocomplete offset-y dense v-model="cotizacion_para_duplicar.has_cliente" :items="clientes" item-text="razon_social" outlined s label="Seleccionar Cliente" return-object></v-autocomplete>
                        </v-col>
                        <v-col cols="12" xs="12" sm="12" md="3" lg="3">
                            <v-autocomplete offset-y dense v-model="cotizacion_para_duplicar.has_moneda" :items="monedas" item-text="clave" outlined s label="Seleccionar Moneda" return-object></v-autocomplete>
                        </v-col>
                        <v-col cols="12" xs="12" sm="12" md="3" lg="3">
                            <v-autocomplete offset-y dense v-model="cotizacion_para_duplicar.has_tiempo_de_entrega" :items="tiempos_de_entrega" item-text="nombre" outlined s label="Tiempo de Entrega" return-object></v-autocomplete>
                        </v-col>
                        <v-col cols="12" xs="12" sm="12" md="3" lg="3">
                        ...
                        <v-col cols="12" xs="12" sm="12" md="3" lg="3">
                            <v-text-field :rules="[rules.required]" v-model="cotizacion_para_duplicar.condicion" dense outlined label="Condiciones" />
                        </v-col>
                        <v-col cols="12" xs="12" sm="12" md="6" lg="6">
                            <v-textarea v-model="cotizacion_para_duplicar.nota_para_la_cotizacion" outlined label="Notas de la cotizacion"></v-textarea>
                        </v-col>
                        <v-col cols="12" xs="12" sm="12" md="6" lg="6">
                            <v-textarea v-model="cotizacion_para_duplicar.nota_de_seguimiento" outlined label="Notas de seguimiento"></v-textarea>
                        </v-col>
                    </v-row>
                </v-form>
            </v-card-text>
            <v-card-text>
                ...
                <v-simple-table>
                    <template v-slot:default>
                        ...
                        <tbody>
                            <tr v-for="(item, p) in cotizacion_para_duplicar.has_partidas" :key="p">
                                <td>
    ...
    computed: {
        ...
        var_computed_iva: {
            get() {
                ...
                this.cotizacion_para_duplicar.has_partidas.forEach((item) => {
                    item_iva = (item.importe * this.cotizacion_para_duplicar.has_cliente.iva) / 100;
                    result += item_iva;
                });
