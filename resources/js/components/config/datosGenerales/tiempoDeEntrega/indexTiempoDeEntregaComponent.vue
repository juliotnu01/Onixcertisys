    ...
    <v-card class="p-2">
      ...
      <v-container>
        <v-toolbar flat color="white">
          <v-text-field
            label="Buscar Tiempo de Entrega"
            placeholder=""
            v-model="search_mag"
            outlined
            small
          ...
          <v-btn
            ...
            dark
            color="primary"
            @click="$store.commit('setDialogTiempoDeEntrega', true)"
          >
...
export default {
  ...
  methods: {
    ...
    async cargarDocumentoTDE() {
      try {
        this.loadingUpload = true;
        await this.services.tiempoDeEntregaServices.agregarTDEMasivamente(this.files[0]);
