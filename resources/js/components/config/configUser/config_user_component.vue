        ...
        <v-row justify="center">
                ...
                <v-card class="p-2">
                    ...
                    <v-container>
                        <v-toolbar flat color="white">
                            <v-text-field label="Buscar Usuario" placeholder="" v-model="search_user" outlined small></v-text-field>
                            <v-spacer></v-spacer>
                        ...
                        <v-data-table :headers="headers_user" :items="listUser" :search="search_user" class="elevation-3">
                            <template v-slot:item.accion="{ item }">
                                <td class="text-center">
                                    <v-btn icon color="red"  small @click="eliminarUsuario(item)" ><v-icon>mdi-delete</v-icon></v-btn>
                                    <v-btn icon color="warning" small @click="editarUsuario(item)"><v-icon>mdi-pencil</v-icon></v-btn>
                                </td>
                            </template>
                            ...
                            <template v-slot:item.add_premiso="{ item }">
                                <td class="text-center">
                                     <v-btn class="mx-2" dark color="green" text block  @click="asignarPermisos(item)" >
                                         Asignar permisos
                                    </v-btn>
                                </td>
                            </template>
            ...
            <v-col cols="12" sm="12" xs="12" md="6" lg="4">
                <v-card class="p-2">
                    ...
                    <v-container>
                        <v-toolbar flat color="white">
                            <v-text-field label="Buscar rol" placeholder="" v-model="search_rol" outlined small></v-text-field>
                            <v-spacer></v-spacer>
                            <v-btn class="mx-2" fab dark color="primary" @click="$store.commit('setRolRegisterModal', true)">
                                <v-icon dark>mdi-plus</v-icon>
                            </v-btn>
                        </v-toolbar>
                        <v-data-table :headers="headers_rol" :items="listRoles" :search="search_rol" class="elevation-3">
                            <template v-slot:item.accion="{ item }">
                                <td class="text-center">
                                    <v-btn icon color="red"  small @click="eliminarRol(item)"><v-icon>mdi-delete</v-icon></v-btn>
                                    <v-btn icon color="warning" small @click="editarRol(item)" ><v-icon>mdi-pencil</v-icon></v-btn>
                                </td>
                            </template>
                             <template v-slot:item.add_user="{ item }">
                                <td class="text-center">
                                    <v-btn class="mx-2" dark color="green" text block @click="asignarRol(item)" >
                                         Asignar rol
                                    </v-btn>
                                </td>
                            </template>
...
export default {
    ...
    data() {
        return {
            ...
            headers_user: [
                ...
                { text: 'Asignar Permisos del rol', align: 'center', sortable: false, value: 'add_premiso' }
            ],
            headers_rol: [
                { text: 'Nombre del rol', align: 'end', sortable: false, sortable: true, value: 'name_rol' },
                { text: 'Acción', align: 'center', sortable: false, value: 'accion' },
                { text: 'Asignar rol a usuario', align: 'center', sortable: false, value: 'add_user' }
            ],
        }
    },
    async mounted() {
    ...
    methods:{
        ...mapActions(['chargeRolSelected', 'chargeUserSelectedRolPermission', 'chargeUserEditar', 'chargeRol']),
